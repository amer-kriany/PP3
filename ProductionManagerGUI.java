...
public class ProductionManagerGUI extends JFrame {
    ...
    class ProductHistoryFrame extends JFrame {
        ...
        public ProductHistoryFrame() {
            ...
            productBox = new JComboBox<>(new String[] { "Laptop", "Phone", "Tablet", "JACKET", "JEANSE", "HOODIE",
                    "tuna", "sardines", "Lanchun" });
    ...
    private void deleteSelectedTask() {
        ...
        if (row != -1) {
            ...
            int confirm = JOptionPane.showConfirmDialog(this, "Delete Task ID: " + taskId + "?", "Confirm",
                    JOptionPane.YES_NO_OPTION);
            if (confirm == JOptionPane.YES_OPTION) {
                for (ProductLine pl : pm.getProductLines())
                    pl.productLineTasks.removeIf(t -> t.taskID == taskId);
                refreshTable();
            }
        } else {
    ...
    public void refreshTable() {
        ...
        for (ProductLine pl : pm.getProductLines())
            synchronized (pl.productLineTasks) {
                allTasks.addAll(pl.productLineTasks);
            }
        ...
        for (Task t : allTasks) {
            ...
            if (matches) {
                String lineName = "N/A";
                for (ProductLine pl : pm.getProductLines())
                    if (pl.productLineTasks.contains(t)) {
                        lineName = pl.getLineName();
                        break;
                    }
                model.addRow(new Object[] { t.taskID, t.getTaskName(), t.getClientName(), prod, t.getQuantity(),
                        t.getStatus(), lineName });
            }
        }
    }
    ...
    private boolean isProductInCategory(String productName, String category) {
        ...
        if (category.equals("CLOTHES"))
            return p.matches("JACKET|JEANSE|HOODIE");
        if (category.equals("TECHNOLOGY"))
            return p.matches("LAPTOP|PHONE|TABLET");
        if (category.equals("CANNED_FOOD"))
            return p.matches("TUNA|SARDINES|LANCHUN");
        return false;
    }
    ...
    class AddTaskDialog extends JDialog {
        ...
        private JComboBox<String> comboProduct = new JComboBox<>(new String[] { "Laptop", "Phone", "Tablet", "JACKET",
                "JEANSE", "HOODIE", "tuna", "sardines", "Lanchun" });
        ...
        public AddTaskDialog(JFrame parent) {
            ...
            add(txtQty);
            add(new JLabel(" Deadline (dd-MM-yyyy HH:mm:ss):"));
            add(txtDeadline);
