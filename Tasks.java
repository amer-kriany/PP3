...
public class Tasks {
    ...
    public void deadLine(String endLine) {
        ...
        // if item is empty of something send an alert of choices(delay the task until
        // the item is here , stop the task or reject it)
        // when starting the task all the item that the task desired is taken store it
        // in DSA
        // when starting the task the item is decreasing or when it finished as you want
        // when the task is finished the product will be added to the product clasa as a
        // new product or if it was an old product just incread the quantity

    }

    @Override
    public String toString() {
        return "Task{client='" + clientName + "', product='" + desireProduct + "', quantity=" + quantity + "}";
    }
}